...
class RR(SchedulerDES):
    def scheduler_func(self, cur_event):
        return sorted(filter(lambda p:p.process_state==ProcessStates.READY,self.processes),key=lambda p:p.remaining_time)[0] if type(self) in [SJF,SRTF] else self.processes[cur_event.process_id]
    def dispatcher_func(self, cur_process):
        time_run_for = cur_process.run_for(self.next_event_time()-self.time if type(self)==SRTF else self.quantum,self.time)
